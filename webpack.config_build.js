...
var { publicPath } = require('./config/config');
var entryPath = __dirname + '/src/pages/';
var clientPath = __dirname + '/dist/client';
const copyHtmlImgAction = function(cateName) {
    console.log('开始处理html 图片copy:' + cateName);
    //html copy
    var catepath = entryPath + cateName;
    copyfiles([catepath + '/*.html', clientPath + '/' + cateName], true, function(err) {
        if (err) {
...
const copyVendorAction = function() {
    ...
    var vendorbuildPath = clientPath + '/vendor';
    fs.mkdir(clientPath, function(err) {
        fs.exists(vendorbuildPath, function(exists) {
            console.log(exists ? 'vendor存在无需更新' : '创建vendor');
            if (!exists) {
                fs.mkdir(vendorbuildPath, function(err) {
                    if (err) {
                        console.log(err);
                        return;
                    } else {
                        copyFolder(vendorPath, vendorbuildPath, function(err) {
                            if (err) {
...
copyVendorAction(); //copy 第三方库到build目录
let buildConfig = Object.assign({}, config, {
    devtool: false,
    output: {
        publicPath: publicPath, //生产环境脚本代码则设置cdn地址 https://j1.58cdn.com.cn/escstatic/hmbird/
        libraryTarget: 'umd',
