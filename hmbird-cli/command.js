...
commander
    ...
    .option(
        '-u, --update',
        '升级脚手架,请cd到项目跟目录下执行 hmbird update命令;注意！！！升级会导致项目package.json文件夹重置,请提前备份！'
    )
    ...
    .option('-0, --output [page]', '导出静态资源，before run hmbird build');
if (commander.init) {
    console.log('初始化项目,在当前文件夹下将会创建一个hmbird工程', commander.init);
}
if (commander.update) {
    console.log('升级脚手架,请cd到项目跟目录下执行 hmbird update命令');
}
// 初始化 <name>工程
